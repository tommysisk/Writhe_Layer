...
from writhe_tools import GraphDataSet, WritheMessage
import argparse
import os
...
class PaiNNBase(torch.nn.Module):
    ...
    def __init__(
            ...
            cross_product: bool = False):
        super().__init__()
        message = CrossMessage if cross_product else Message
        layers = []
        for _ in range(n_layers):
            ...
            if writhe_layer:
                layers.append(WritheMessage(n_atoms=20, n_features=n_features, batch_size=batch_size))
...
def main(data_path: str,
    ...
    dataset = GraphDataSet(file=data_path)
    ...
    if ckpt_file is None:
        ...
        model = GeometricDDPM.load_from_checkpoint(ckpt_file)
